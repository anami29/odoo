...
class ZSalesRegisterWizard(models.TransientModel):
    _name = 'z.sales.register.wizard'
    _description = 'Z Sales Register Wizard'
    ...
    def generate_report(self):
        ...
        sheet = workbook.add_worksheet('Z Sales Register Report')
        ...
        # Columns definition
        headers = [
            'Company', 'Invoice Number', 'Invoice Date', 'Accounting Date', 'Customer Name',
            'Customer Tax ID', 'Customer GST Treatment', 'Journal Name', 'Reference Number', 'Customer PAN',
            'Invoice Description', 'Account Name', 'HSN Code', 'Product HSN/SAC Description', 'Unit of Measure',
        ...
        for move in moves:
            ...
            # 6. Customer Tax ID (GSTIN)
            customer_tax_id = move.commercial_partner_id.vat or ''
            # 7. Customer GST Treatment
            gst_treatment = ''
            ...
            if hasattr(partner, 'l10n_in_pan') and partner.l10n_in_pan:
                customer_pan = partner.l10n_in_pan
            elif partner.vat and len(partner.vat) >= 12:
                customer_pan = partner.vat[2:12]
            ...
            for line in move.invoice_line_ids.filtered(lambda l: l.display_type == 'product'):
                ...
                credit_amount = subtotal_amount if move.move_type == 'out_invoice' else 0.0
                debit_amount = subtotal_amount if move.move_type == 'out_refund' else 0.0
                ...
                sheet.write(row_num, 1, invoice_num, text_format)
                sheet.write(row_num, 2, invoice_date, text_format)
                sheet.write(row_num, 3, accounting_date, text_format)
                sheet.write(row_num, 4, customer_name, text_format)
                sheet.write(row_num, 5, customer_tax_id, text_format)
                ...
                sheet.write(row_num, 9, customer_pan, text_format)
        ...
        filename = f"Z Sales Register Report - {date_from_str} to {date_to_str}.xlsx"
