...
class HelpdeskTicketReportAht(models.Model):
    ...
    category_id = fields.Many2one("helpdesk.ticket.category", string="Category", readonly=True)
    ...
    resolution_hours = fields.Float(
        string="Resolution Time (Hours)", readonly=True, group_operator="avg"
    )
    assign_hours = fields.Float(
        string="First Assignment Time (Hours)", readonly=True, group_operator="avg"
    )
        ...
        self.env.cr.execute(
            ...
            CREATE OR REPLACE VIEW {self._table} AS (
                SELECT
                    ...
                    CASE WHEN t.closed_date IS NOT NULL AND t.create_date IS NOT NULL THEN
                        EXTRACT(EPOCH FROM (t.closed_date - t.create_date)) / 3600.0
                    ELSE
                        NULL
                    END AS resolution_hours,
                    CASE WHEN t.assigned_date IS NOT NULL AND t.create_date IS NOT NULL THEN
                        EXTRACT(EPOCH FROM (t.assigned_date - t.create_date)) / 3600.0
                    ELSE
                        NULL
                    END AS assign_hours,
                    s.closed AS is_closed,
                    COALESCE(
                        BOOL_OR(tsla.state = 'expired' OR (tsla.state = 'in_progress' AND tsla.deadline < NOW())),
                        FALSE
                    ) AS sla_breached
                FROM
                    helpdesk_ticket t
                    LEFT JOIN helpdesk_ticket_stage s ON t.stage_id = s.id
                    LEFT JOIN helpdesk_ticket_sla tsla ON t.id = tsla.ticket_id
                WHERE
                    t.active = TRUE
                GROUP BY
                    t.id, s.closed
